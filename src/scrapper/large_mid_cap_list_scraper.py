...
url = mutual_funds_urls["large-and-mid-cap"]
...
def get_list(is_testing: bool = True):
    ...
    content = None
    if is_testing:
        content = open("test_input/large-mid-cap-equity-funds.html", encoding="utf8")
    else:
    ...
    for row in table_body.find_all('tr'):
        time.sleep(3)
        # scrape_each_item(data=data, funds_urls=funds_urls, category="large-and-mid-cap", row=row)
        fund_info = {}
